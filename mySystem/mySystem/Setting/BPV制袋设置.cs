...
namespace mySystem.Setting
{
    public partial class BPV制袋设置 : BaseForm
    {
        ...
        {
            //**************************   开机    ***********************************
            dt开机 = new DataTable("设置生产前确认项目"); //""中的是表名
            da开机 = new SqlDataAdapter("select * from 设置生产前确认项目", mySystem.Parameter.conn);
            ...
            readDGVWidthFromSettingAndSet(dgv清场);
            ...
            da产品 = new SqlDataAdapter("select * from 设置BPV产品", mySystem.Parameter.conn);
            ...
            //**************************   产品编码    ***********************************
            dt产品编码 = new DataTable("设置BPV产品编码"); //""中的是表名
            da产品编码 = new SqlDataAdapter("select * from 设置BPV产品编码", mySystem.Parameter.conn);
            ...
            //**************************   产品规格    ***********************************
            dt产品规格 = new DataTable("设置BPV制袋产品规格"); //""中的是表名
            da产品规格 = new SqlDataAdapter("select * from 设置BPV制袋产品规格", mySystem.Parameter.conn);
            ...
            setDGV产品规格Column();
            readDGVWidthFromSettingAndSet(dgv产品规格);

            //**************************   封边    ***********************************
            dt封边 = new DataTable("设置BPV制袋封边"); //""中的是表名
            da封边 = new SqlDataAdapter("select * from 设置BPV制袋封边", mySystem.Parameter.conn);
            ...
            readDGVWidthFromSettingAndSet(dgv封边);

            //**************************   工艺    ***********************************
            dt工艺 = new DataTable("设置BPV制袋工艺"); //""中的是表名
            da工艺 = new SqlDataAdapter("select * from 设置BPV制袋工艺", mySystem.Parameter.conn);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (b)
                    {
                        ...
                        Boolean c = checkPeopleRight(); //判断用户是否在BPV制袋用户表中
                        if (c)
                        {
        ...
        private Boolean EachPeopleRightCheck(String name)
        {
            ...
            {
                b = false;
                MessageBox.Show("员工" + "“" + name + "”" + "无操作BPV制袋工序权限，保存失败！");
            }
        ...
        private void dgv开机_ColumnWidthChanged(object sender, DataGridViewColumnEventArgs e)
        {
