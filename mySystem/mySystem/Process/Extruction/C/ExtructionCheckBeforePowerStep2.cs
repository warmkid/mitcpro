    ...
    public partial class ExtructionCheckBeforePowerStep2 : BaseForm
    {
        ...
        private void getPeople()
        {
            ...
            if (!mySystem.Parameter.isSqlOk)
            {
                ...
                da = new OleDbDataAdapter("select * from 用户权限 where 步骤='吹膜开机前确认表'", connOle);
            ...
            else
            {
                ...
                ls操作员 = new List<string>();
                ls审核员 = new List<string>();
                da = new SqlDataAdapter("select * from 用户权限 where 步骤='吹膜开机前确认表'", conn);
                dt = new DataTable("temp");
                da.Fill(dt);
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            else
            {
                bs记录详情 = new BindingSource();
                dt记录详情 = new DataTable(tableInfo);
                da记录详情_sql = new SqlDataAdapter("select * from " + tableInfo + " where T吹膜机组开机前确认表ID = " + ID.ToString(), conn);
                cb记录详情_sql = new SqlCommandBuilder(da记录详情_sql);
                da记录详情_sql.Fill(dt记录详情);
            }
        }
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                DataTable dt_temp = new DataTable("待审核");
                //BindingSource bs_temp = new BindingSource();
                SqlDataAdapter da_temp = new SqlDataAdapter("select * from 待审核 where 表名='吹膜开机前确认表' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.conn);
        ...
        private void bt查看人员信息_Click(object sender, EventArgs e)
        {
            if (!mySystem.Parameter.isSqlOk)
            {
                OleDbDataAdapter da;
                DataTable dt;
                da = new OleDbDataAdapter("select * from 用户权限 where 步骤='吹膜开机前确认表'", mySystem.Parameter.connOle);
                dt = new DataTable("temp");
                da.Fill(dt);
            ...
            else
            {
                SqlDataAdapter da;
                DataTable dt;
                da = new SqlDataAdapter("select * from 用户权限 where 步骤='吹膜开机前确认表'", mySystem.Parameter.conn);
                dt = new DataTable("temp");
                da.Fill(dt);
                String str操作员 = dt.Rows[0]["操作员"].ToString();
                String str审核员 = dt.Rows[0]["审核员"].ToString();
