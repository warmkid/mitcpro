...
namespace mySystem.Extruction.Process
{
    public partial class ExtructionpRoductionAndRestRecordStep6 : BaseForm
    {
        ...
        private void getPeople()
        {
            ...
            {
                SqlDataAdapter da;
                da = new SqlDataAdapter("select * from 用户权限 where 步骤='吹膜生产和检验记录表'", conn);
                da.Fill(dt);
            }
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            else
            {
                bs记录详情 = new BindingSource();
                dt记录详情 = new DataTable(tableInfo);
                da记录详情_sql = new SqlDataAdapter("select * from " + tableInfo + " where T吹膜工序生产和检验记录ID = " + ID.ToString(), conn);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            if (DialogResult.Yes == MessageBox.Show("确认本表已经填完吗？提交审核之后不可修改", "提示", MessageBoxButtons.YesNo))
            {
                ...
                else
                {
                    ...
                    DataTable dt_temp = new DataTable("待审核");
                    //BindingSource bs_temp = new BindingSource();
                    SqlDataAdapter da_temp = new SqlDataAdapter("select * from 待审核 where 表名='吹膜生产和检验记录表' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.conn);
