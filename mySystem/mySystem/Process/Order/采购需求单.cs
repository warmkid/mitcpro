...
//        OleDbConnection conn;
        ...
        private void getOtherData()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("select * from 设置产成品存货档案", mySystem.Parameter.conn);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        {
            daOuter = new SqlDataAdapter("select * from 采购需求单 where 用途='" + yongtu + "'", mySystem.Parameter.conn);
        ...
        {
            daInner = new SqlDataAdapter("select * from 采购需求单详细信息 where 采购需求单ID=" + id, mySystem.Parameter.conn);
        ...
        private void getPeople()
        {
            ...
            da = new SqlDataAdapter("select * from 订单用户权限 where 步骤='采购需求单'", mySystem.Parameter.conn);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da_temp = new SqlDataAdapter(@"select * from 待审核 where 表名='采购需求单' and 对应ID=" + (int)dtOuter.Rows[0]["ID"], mySystem.Parameter.conn);
        ...
        private void 采购需求单_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            try
            {
                (this.Owner as 订单和库存管理.订单管理).btn查询采购需求单.PerformClick();
            }
        ...
        public int print(bool isShow)
        {
            ...
            // 利用这个进程打开一个Excel文件
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\订单\表90. PALL AUSTAR采购需求单.xlsx");
        ...
        private Microsoft.Office.Interop.Excel._Worksheet printValue(Microsoft.Office.Interop.Excel._Worksheet mysheet, Microsoft.Office.Interop.Excel._Workbook mybook)
        {
            ...
            mysheet.Cells[10 + ind, 3].Value = Convert.ToDateTime(dtOuter.Rows[0]["申请日期"]).ToString("D");
            mysheet.Cells[10 + ind, 4].Value = "申请人：" + dtOuter.Rows[0]["申请人"].ToString();
            //mysheet.Cells[10 + ind, 6].Value = dtOuter.Rows[0]["申请部门"].ToString();
            mysheet.Cells[10 + ind, 9].Value = dtOuter.Rows[0]["审核人"].ToString();
            mysheet.Cells[10 + ind, 11].Value = Convert.ToDateTime(dtOuter.Rows[0]["审核日期"]).ToString("D");
