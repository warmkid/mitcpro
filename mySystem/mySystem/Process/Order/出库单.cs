...
//        OleDbConnection conn;
        ...
        SqlDataAdapter daOuter, daInner,da库存信息;
        SqlCommandBuilder cbOuter, cbInner, cb库存信息;
            ...
            : base(mainform)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("select * from 销售订单 where 订单号='" + dtOuter.Rows[0]["销售订单号"].ToString() + "'", mySystem.Parameter.conn);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("select 订单号 from 销售订单 where 状态='审核完成' OR 状态='已生成采购需求单'", mySystem.Parameter.conn);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        private void getPeople()
        {
            ...
            da = new SqlDataAdapter("select * from 订单用户权限 where 步骤='出库单'", mySystem.Parameter.conn);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("select * from 销售订单 where 订单号='" + tb销售订单号.Text + "'", mySystem.Parameter.conn);
            DataTable dt = new DataTable();
            da.Fill(dt);
            if (dt.Rows.Count == 0)
            {
        ...
        {
            daOuter = new SqlDataAdapter("select * from 出库单 where 出库单号='" + p + "'", mySystem.Parameter.conn);
        ...
        {
            daOuter = new SqlDataAdapter("select * from 出库单 where ID=" + id, mySystem.Parameter.conn);
            cbOuter = new SqlCommandBuilder(daOuter);
            dtOuter = new DataTable("出库单");
            bsOuter = new BindingSource();
        ...
        {
            daInner = new SqlDataAdapter("select * from 出库单详细信息 where 出库单ID=" + id, mySystem.Parameter.conn);
        ...
        private DataTable get未发货信息(int id, string dingdanhao)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("select * from 销售订单详细信息 where 销售订单ID=" + id, mySystem.Parameter.conn);
            DataTable dt = new DataTable();
            da.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
            ...
            // 拿到对本订单的出库单详细信息
            // 计算剩余未出库量
            da = new SqlDataAdapter("select * from 出库单详细信息 where 用途='" + dingdanhao + "' and 状态='已出库'", mySystem.Parameter.conn);
            dt = new DataTable();
            da.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
        ...
        {
            //da库存信息 = new SqlDataAdapter("select ID,仓库名称,产品代码,产品批号,现存数量,用途,冻结状态 from 库存台帐 where 用途='" + dingdanhao + "' and 状态='合格'", conn);
            //cb库存信息 = new SqlCommandBuilder(da库存信息);
            ...
            string 库存sql = @"select ID, 仓库名称,产品代码,产品名称,产品规格,产品批号,现存数量,用途,冻结状态 from (select * from 库存台帐 where 状态='合格') t where 产品代码='{0}'";
            ...
            库存sql += " order by 产品代码";
            da库存信息 = new SqlDataAdapter(库存sql, mySystem.Parameter.conn);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da_temp = new SqlDataAdapter(@"select * from 待审核 where 表名='出库单' and 对应ID=" + (int)dtOuter.Rows[0]["ID"], mySystem.Parameter.conn);
