...
namespace mySystem.Process.Stock
{
    public partial class 产品退货评审单1 : BaseForm
    {
        ...
        {
            daOuter = new OleDbDataAdapter("select * from 产品退货评审单1 where 退货申请单编号='" + code + "'", conn);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            da = new OleDbDataAdapter("select * from 待审核 where 表名='产品退货评审单1' and 对应ID=" + _id, conn);
        ...
        public override void CheckResult()
        {
            ...
            {
                // 自动生产评审单2
                da = new OleDbDataAdapter("select * from 产品退货评审单2 where 退货申请单编号='" + _code + "'", conn);
                cb = new OleDbCommandBuilder(da);
                dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
