...
namespace mySystem.Process.Bag.BTV
{
    public partial class 产品热合强度检验记录 : BaseForm
    {
        ...
        void readOuterData()
        {
            ...
            daOuter = new OleDbDataAdapter(String.Format(sql, mySystem.Parameter.bpvbagInstruID, nowString), conn);
            cbOuter = new OleDbCommandBuilder(daOuter);
            dtOuter = new DataTable("产品热合强度检验记录");
            bsOuter = new BindingSource();
        ...
        public void print(bool isShow)
        {
            ...
            // 利用这个进程打开一个Excel文件
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\BPVBag\QB-PA-PP-03-R02A 产品热合强度检验记录.xlsx");
