...
namespace mySystem.Process.Bag.BTV
{
    public partial class BTVAssemblyConfirm : BaseForm
    {
        ...
        private void getPeople()
        {
            ...
            da = new OleDbDataAdapter("select * from 用户权限 where 步骤='BPV装配确认记录'", connOle);
        ...
        private void outerBind()
        {
            ...
            tb产品代码.DataBindings.Add("Text", bs记录.DataSource, "成品代码");
        ...
        private DataRow writeOuterDefault(DataRow dr)
        {
            ...
            dr["备注"] = "";
            dr["审核员"] = "";
            //dr["审核日期"] = Convert.ToDateTime(dtp生产日期.Value.ToString("yyyy/MM/dd"));
            dr["审核是否通过"] = false;
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            da记录详情 = new OleDbDataAdapter("select * from " + tableInfo + " where TBPV装配确认记录ID = " + ID.ToString(), connOle);
        ...
        private DataRow writeInnerDefault(Int32 ID, DataRow dr)
        {
            ...
            dr["产品代码"] = dt代码批号.Rows[0]["产品代码"];
            dr["组件数量"] = 0;
            dr["合格数量只"] = 0;
            dr["不良品数量只"] = 0;
            dr["质量检查判定"] = "";
            dr["生产操作员"] = mySystem.Parameter.userName;
            dr["完成时间"] = Convert.ToDateTime(DateTime.Now.ToString());
            dr["QC人员"] = "";
            dr["确认时间"]=Convert.ToDateTime(DateTime.Now.ToString());
            return dr;
        }
        ...
        private void setDataGridViewFormat()
        {
            ...
            dataGridView1.Columns["TBPV装配确认记录ID"].Visible = false;
            //不可用
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            OleDbDataAdapter da_temp = new OleDbDataAdapter("select * from 待审核 where 表名='BPV装配确认记录' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.connOle);
        ...
        public void print(bool preview)
        {
            ...
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\BPVBag\SOP-MFG-306-R05A  BPV装配确认记录.xlsx");
        ...
        private void getNum(Int32 Rownum)
        {
            ...
            // 膜卷长度求和
            if ((Int32.TryParse(dt记录详情.Rows[Rownum]["组件数量"].ToString(), out numA) == true) && (Int32.TryParse(dt记录详情.Rows[Rownum]["合格数量只"].ToString(), out numB) == true) && (Int32.TryParse(dt记录详情.Rows[Rownum]["不良品数量只"].ToString(), out numC) == true) )
            {
                //均为数值类型
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0)
            {
                if (dataGridView1.Columns[e.ColumnIndex].Name == "组件数量")
                { getNum(e.RowIndex); }
                else if (dataGridView1.Columns[e.ColumnIndex].Name == "合格数量只")
                { getNum(e.RowIndex); }
                else if (dataGridView1.Columns[e.ColumnIndex].Name == "不良品数量只")
                { getNum(e.RowIndex); }
                /*
