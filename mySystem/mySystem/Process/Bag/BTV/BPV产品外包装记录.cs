...
{
    public partial class BPV产品外包装记录 : mySystem.BaseForm
    {
        ...
        public BPV产品外包装记录(mySystem.MainForm mainform, Int32 ID) : base(mainform)
        {
            InitializeComponent();
        ...
        private void getPeople()
        {
            ...
            da = new OleDbDataAdapter("select * from 用户权限 where 步骤='产品外包装记录表'", connOle);
        ...
        private void getOtherData()
        {
            ...
            if (!isSqlOk)
            {
                ...
                {
                    //外面的信息填写
                    lb纸箱代码.Text = reader1["外包物料代码1"].ToString();
                    lb纸箱批号.Text = reader1["外包物料批号1"].ToString();
                    lb标签代码.Text = reader1["外包物料代码3"].ToString();
                    //读取详细信息
        ...
        public override void CheckResult()
        {
            ...
            if (checkform.ischeckOk)
            {
                ...
                da = new OleDbDataAdapter(string.Format(sql, dt记录.Rows[0]["产品代码"].ToString()), Tconn);
                dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count == 0)
                {
                ...
                da = new OleDbDataAdapter(string.Format(sql, dt记录.Rows[0]["产品代码"].ToString(), 订单号), Tconn);
        ...
        public void print(bool isShow)
        {
            ...
            // 利用这个进程打开一个Excel文件
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\BPVBag\SOP-MFG-111-R01A 产品外包装记录.xlsx");
