...
{
    public partial class BTVRunningRecordRHJsingle : BaseForm
    {
        private String table = "单管口热合机运行记录";
        private String tableInfo = "单管口热合机运行记录详细信息"; 
        ...
        private void getPeople()
        {
            ...
            da = new OleDbDataAdapter("select * from 用户权限 where 步骤='单管口热合机运行记录'", connOle);
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            da记录详情 = new OleDbDataAdapter("select * from " + tableInfo + " where T单管口热合机运行记录ID = " + ID.ToString(), connOle);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            OleDbDataAdapter da_temp = new OleDbDataAdapter("select * from 待审核 where 表名='单管口热合机运行记录' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.connOle);
        ...
        public void print(bool preview)
        {
            ...
            for (int i = 0; i < (rowNumTotal > rowNumPerSheet ? rowNumPerSheet : rowNumTotal); i++)
            {
                ...
                my.Cells[i + 8, 8].Value = dt记录详情.Rows[i]["电压"];
                my.Cells[i + 8, 9].Value = dt记录详情.Rows[i]["外观检查"];
                my.Cells[i + 8, 10].Value = dt记录详情.Rows[i]["生产合格品数量"];
                my.Cells[i + 8, 11].Value = dt记录详情.Rows[i]["不良品数量"];
                my.Cells[i + 8, 12].Value = dt记录详情.Rows[i]["操作员"];
                my.Cells[i + 8, 13].Value = dt记录详情.Rows[i]["备注"];
            ...
            my.Cells[20 + varOffset, 12].Value = "审核员/日期： " + dt记录.Rows[0]["审核员"] + Convert.ToDateTime( dt记录.Rows[0]["审核日期"]).ToString("yyyy年MM月dd日");
            if (preview)
            {
                my.Select();
