...
namespace mySystem.Process.Bag.BTV
{
    public partial class BTVRunningRecordRHJsingle : BaseForm
    {
        ...
        private void getPeople()
        {
            ...
            da = new SqlDataAdapter("select * from 用户权限 where 步骤='单管口热合机运行记录'", mySystem.Parameter.conn);
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            da记录详情 = new SqlDataAdapter("select * from " + tableInfo + " where T单管口热合机运行记录ID = " + ID.ToString(), mySystem.Parameter.conn);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da_temp = new SqlDataAdapter("select * from 待审核 where 表名='单管口热合机运行记录' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.conn);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("select * from " + table + " where 生产指令ID=" + _InstruID, mySystem.Parameter.conn);
        ...
        private void BTVRunningRecordRHJsingle_FormClosing(object sender, FormClosingEventArgs e)
        {
