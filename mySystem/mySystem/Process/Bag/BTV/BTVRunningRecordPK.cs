...
{
    public partial class BTVRunningRecordPK : BaseForm
    {
        /// <summary>
        /// 瓶口焊接机运行记录
        /// </summary>
        ...
        public BTVRunningRecordPK(MainForm mainform, Int32 ID) : base(mainform)
        {
            InitializeComponent();
        ...
        public int print(bool preview)
        {
            ...
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\BPVBag\7 SOP-MFG-418-R01A 瓶口焊接机运行记录.xlsx");
            ...
            my.Cells[3, 20].Value = "生产指令编号：" + dt记录.Rows[0]["生产指令编号"];
            ...
            //my.Cells[19 + varOffset, 18].Value = "合格品数量： " + dt记录.Rows[0]["合格品数量"] + " 个，\n不良品数量： " + dt记录.Rows[0]["不良品数量"] + " 个。";
            //my.Cells[19 + varOffset, 23].Value = "审核员： " + dt记录.Rows[0]["审核员"] + "\n日期： " + Convert.ToDateTime(dt记录.Rows[0]["审核日期"]).ToString("yyyy年MM月dd日");
            if (preview)
            {
                my.Select();
