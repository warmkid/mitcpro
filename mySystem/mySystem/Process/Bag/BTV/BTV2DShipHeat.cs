...
namespace mySystem.Process.Bag.BTV
{
    public partial class BTV2DShipHeat : BaseForm
    {
        ...
        public BTV2DShipHeat(MainForm mainform)
            : base(mainform)
        {
            InitializeComponent();
        ...
        public BTV2DShipHeat(MainForm mainform, int ID)
            : base(mainform)
        {
        ...
        private void getPeople()
        {
            ...
            da = new SqlDataAdapter("select * from 用户权限 where 步骤='2D袋体与船型接口热合记录';", mySystem.Parameter.conn);
        ...
        private void getTotal()
        {
            ...
            for (int i = 0; i < dt记录详情.Rows.Count; i++)
            {
                if (Int32.TryParse(dt记录详情.Rows[i]["合格产品数量"].ToString(), out numtemp) == true)
                { sum[0] += numtemp; }
            }
        ...
        private bool TextBox_check()
        {
            ...
            List<TextBox> TextBoxList = new List<TextBox>(new TextBox[] { tb不良品数量});
            List<String> StringList = new List<String>(new String[] { "不良品数量"});
            int numtemp = 0;
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0)
            {
                if (dataGridView1.Columns[e.ColumnIndex].Name == "合格产品数量")
                {
                    getTotal();
                }
        ...
        private void BTV2DShipHeat_FormClosing(object sender, FormClosingEventArgs e)
        {
