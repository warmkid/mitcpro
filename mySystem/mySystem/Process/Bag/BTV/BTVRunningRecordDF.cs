...
{
    public partial class BTVRunningRecordDF : BaseForm
    {


        /*
         * READ ME : NEED TO MODIFY DATABASE AS FOLLOW:
         * ADD 生产时间 COLUMN IN 底封机运行记录详细信息
         * REMOVE 设定 COLUMN IN 底封机运行记录详细信息( ACTUALLY DO NOT HAVE TO)
         */
        ...
        public BTVRunningRecordDF(MainForm mainform, int ID)
            : base(mainform)
        {
            InitializeComponent();
        ...
        private void readOuterData(Int32 InstruID, String MoProductCode, DateTime searchTime)
        {
            ...
            da记录 = new OleDbDataAdapter("select * from " + table + " where 生产指令ID = " + InstruID.ToString() + " and 膜或袋体代码 = '" + MoProductCode + "' and 生产日期 = #" + searchTime.ToString("yyyy/MM/dd") + "# ", connOle);
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            da记录详情 = new OleDbDataAdapter("select * from " + tableInfo + " where T底封机运行记录ID = " + ID.ToString(), connOle);
        ...
        /// <summary>
        /// this function has been updated to latest edition responding to new documents
        /// by pool on 2017-11-02
        ...
        public void print(bool preview)
        {
            ...
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\BPVBag\8 SOP-MFG-412-R01A 底封机运行记录.xlsx");
            ...
            int rowStartAt = 8;
            //my.Cells[3, 1].Value = "膜代码：" + dt记录.Rows[0]["膜代码"];
            my.Cells[3, 6].Value = "产品代码：" + dt记录.Rows[0]["产品代码"];
            my.Cells[3, 12].Value = "产品批号：" + dt记录.Rows[0]["产品批号"];
            my.Cells[3, 17].Value = "生产指令编号：" + dt记录.Rows[0]["生产指令编号"];
            ...
            //EVERY SHEET CONTAINS 10 RECORDS
            ...
            for (int i = 0; i < rowNumTotal; i++)
            {
                ...
                my.Cells[i + rowStartAt, 19].Value = dt记录详情.Rows[i]["操作员"];
                my.Cells[i + rowStartAt, 20].Value = dt记录详情.Rows[i]["审核员"];
