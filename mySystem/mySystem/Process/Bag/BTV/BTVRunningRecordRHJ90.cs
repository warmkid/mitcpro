...
namespace mySystem.Process.Bag.BTV
{
    public partial class BTVRunningRecordRHJ90 : BaseForm
    {
        ...
        private void getPeople()
        {
            ...
            da = new SqlDataAdapter("select * from 用户权限 where 步骤='90度热合机运行记录'", mySystem.Parameter.conn);
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            da记录详情 = new SqlDataAdapter("select * from [" + tableInfo + "] where T90度热合机运行记录ID = " + ID.ToString(), mySystem.Parameter.conn);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da_temp = new SqlDataAdapter("select * from 待审核 where 表名='90度热合机运行记录' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.conn);
        ...
        public int print(bool preview)
        {
            ...
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\BPVBag\12 SOP-MFG-505-R01A  90度热合机运行记录1.xlsx");
            ...
            int rowStartAt = 8;
            //my.Cells[3, 15].Value = "膜或袋体代码："+ dt记录.Rows[0]["膜代码"];
            my.Cells[3, 1].Value = "产品代码：" + dt记录.Rows[0]["产品代码"];
            my.Cells[3, 8].Value = "产品批号：" + dt记录.Rows[0]["产品批号"];
            my.Cells[3, 22].Value = "生产指令编号：" + dt记录.Rows[0]["生产指令编号"];
            //my.Cells[3, 22].Value = "生产日期："+ Convert.ToDateTime(dt记录.Rows[0]["生产日期"]).ToString("yyyy年MM月dd日");
