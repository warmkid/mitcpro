...
namespace mySystem.Process.Bag
{
    public partial class CSBag_InnerPackaging : BaseForm
    {
        ...
        private void getPeople()
        {
            ...
            OleDbDataAdapter datemp = new OleDbDataAdapter("select * from 用户权限 where 步骤 = '产品内包装记录'", connOle);
            datemp.Fill(dtusers);
            datemp.Dispose();
        ...
        private void setEnableReadOnly()
        {
            ...
            {
                if (stat_form == 3 || stat_form == 2)  //2审核通过||3审核未通过
                {
                    //控件都不能点，只有打印,日志可点
                    setControlFalse();
                }
                else if (stat_form == 0)//0未保存
                {
                    //控件都不能点，只有打印,日志可点
                    setControlFalse();
        ...
        private void btn查询新建_Click(object sender, EventArgs e)
        {
            ...
            { DataShow(mySystem.Parameter.csbagInstruID, cb产品代码.Text.ToString()); }
        }
        ...
        private bool Save()
        {
            ...
            else
            {
                ...
                bs记录.EndEdit();
                da记录.Update((DataTable)bs记录.DataSource);
                readOuterData(mySystem.Parameter.csbagInstruID, cb产品代码.Text);
                outerBind();
