...
{
    public partial class LDPE生产领料申请单 : BaseForm
    {
        ...
        public LDPE生产领料申请单(mySystem.MainForm mainform,BaseForm owner)
            : base(mainform)
        {
            InitializeComponent();
        ...
        public LDPE生产领料申请单(mySystem.MainForm mainform, Int32 ID,BaseForm owner)
            : base(mainform)
        {
            InitializeComponent();
            conn = Parameter.conn;
        ...
        private void getOtherData()
        {
            ...
            if (isSqlOk)
            {
                ...
                if (dt生产指令.Rows.Count > 0)
                {
                    ...
                    { dt生产指令信息.Rows.Add(new object[] { InstruID, Instruction, "LDPE制袋", dttemp2.Rows[0]["产品代码"], dttemp2.Rows[0]["产品批号"], isSqlOk }); }
                    else
                    { dt生产指令信息.Rows.Add(new object[] { InstruID, Instruction, "LDPE制袋", "暂无", "暂无", isSqlOk }); }
                }
                else
                {
                    //添加物料简称、批号、代码
