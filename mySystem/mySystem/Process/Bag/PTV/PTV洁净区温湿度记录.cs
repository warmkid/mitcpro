...
{
    public partial class PTV洁净区温湿度记录 : mySystem.BaseForm
    {        
        private String table = "洁净区温湿度记录表";
        private String tableInfo = "洁净区温湿度记录详细信息";
        ...
        public PTV洁净区温湿度记录(mySystem.MainForm mainform) : base(mainform)
        {
            InitializeComponent();
        ...
        public PTV洁净区温湿度记录(mySystem.MainForm mainform, Int32 ID)  : base(mainform)
        {
            InitializeComponent();
        ...
        private void getPeople()
        {
            ...
            da = new SqlDataAdapter("select * from 用户权限 where 步骤='洁净区温湿度记录表'", mySystem.Parameter.conn);
        ...
        private void outerBind()
        {
            ...
            lb生产工序.DataBindings.Clear();
            lb生产工序.DataBindings.Add("Text", bs记录.DataSource, "生产工序");
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            da记录详情 = new SqlDataAdapter("select * from " + tableInfo + " where T洁净区温湿度记录ID = " + ID.ToString(), mySystem.Parameter.conn);
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da_temp = new SqlDataAdapter("select * from 待审核 where 表名='洁净区温湿度记录表' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.conn);
        ...
        public int print(bool isShow)
        {
            ...
            // 利用这个进程打开一个Excel文件
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\PTV\SOP-MFG-201-R01A 洁净区温湿度记录.xlsx");
        ...
        private Microsoft.Office.Interop.Excel._Worksheet printValue(Microsoft.Office.Interop.Excel._Worksheet mysheet, Microsoft.Office.Interop.Excel._Workbook mybook)
        {
            ...
            //无需插入的部分
            for (int i = 0; i < (rownum > 28 ? 28 : rownum); i++)
            {
                mysheet.Cells[5 + i, 1].Value = Convert.ToDateTime(dt记录详情.Rows[i]["生产日期"].ToString()).ToString("yyyy/MM/dd");
                mysheet.Cells[5 + i, 2].Value = dt记录详情.Rows[i]["班次"].ToString();
                mysheet.Cells[5 + i, 3].Value = Convert.ToDateTime(dt记录详情.Rows[i]["时间"].ToString()).ToString("HH:mm:ss");
                mysheet.Cells[5 + i, 4].Value = dt记录详情.Rows[i]["温度"].ToString();
                mysheet.Cells[5 + i, 5].Value = dt记录详情.Rows[i]["相对湿度"].ToString();
                mysheet.Cells[5 + i, 6].Value = dt记录详情.Rows[i]["判定"].ToString();
                mysheet.Cells[5 + i, 7].Value = dt记录详情.Rows[i]["操作员"].ToString();
                mysheet.Cells[5 + i, 8].Value = dt记录详情.Rows[i]["操作员备注"].ToString();
                mysheet.Cells[5 + i, 9].Value = dt记录详情.Rows[i]["审核员"].ToString();
            }
            //需要插入的部分
            if (rownum > 28)
            {
                for (int i = 28; i < rownum; i++)
                {
        ...
        private void PTV洁净区温湿度记录_FormClosing(object sender, FormClosingEventArgs e)
        {
