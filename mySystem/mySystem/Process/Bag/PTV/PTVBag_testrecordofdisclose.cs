...
{
    public partial class PTVBag_testrecordofdisclose : BaseForm
    {
        private String table = "泄漏测试记录";
        private String tableInfo = "泄漏测试记录详细信息";
        ...
        public PTVBag_testrecordofdisclose(MainForm mainform) : base(mainform)
        {
            InitializeComponent();
            ...
            else
            {
                ...
                DataShow(InstruID, prodCode);
            }
        }

        public PTVBag_testrecordofdisclose(MainForm mainform, Int32 ID)
            : base(mainform)
        {
            InitializeComponent();
        ...
        private void getPeople()
        {
            ...
            da = new SqlDataAdapter("select * from 用户权限 where 步骤='泄露测试记录'", mySystem.Parameter.conn);
        ...
        private void readOuterData(Int32 InstruID, String ProductCode)
        {
            ...
            //da记录 = new SqlDataAdapter("select * from " + table + " where 生产指令ID = " + InstruID.ToString() + " and 产品代码 = '" + ProductCode + "' and 生产日期 = #" + searchTime.ToString("yyyy/MM/dd") + "# ", mySystem.Parameter.conn);
        ...
        private void outerBind()
        {
            ...
            tb参数1.DataBindings.Add("Text", bs记录.DataSource, "prefill");
            tb参数2.DataBindings.Clear();
            tb参数2.DataBindings.Add("Text", bs记录.DataSource, "charde");
            tb参数3.DataBindings.Clear();
            tb参数3.DataBindings.Add("Text", bs记录.DataSource, "settle");
            tb参数4.DataBindings.Clear();
            tb参数4.DataBindings.Add("Text", bs记录.DataSource, "test");
            tb参数5.DataBindings.Clear();
            tb参数5.DataBindings.Add("Text", bs记录.DataSource, "finel");
            tb参数6.DataBindings.Clear();
            tb参数6.DataBindings.Add("Text", bs记录.DataSource, "finer");
            tb参数7.DataBindings.Clear();
            tb参数7.DataBindings.Add("Text", bs记录.DataSource, "flow");
        ...
        private void readInnerData(Int32 ID)
        {
            ...
            da记录详情 = new SqlDataAdapter("select * from " + tableInfo + " where T泄漏测试记录ID = " + ID.ToString(), mySystem.Parameter.conn);
        ...
        private void setDataGridViewFormat()
        {
            ...
            dataGridView1.Columns["T泄漏测试记录ID"].Visible = false;
            dataGridView1.Columns["序号"].ReadOnly = true;
            dataGridView1.Columns["复核人"].ReadOnly = true;
            dataGridView1.Columns["操作员备注"].Visible = false;


            dataGridView1.Columns["prefill"].HeaderText = "PREFILL\r(%)";
            dataGridView1.Columns["charde"].HeaderText = "CHARDE\r(s)";
            dataGridView1.Columns["settle"].HeaderText = "SETTLE\r(s)";
            dataGridView1.Columns["test"].HeaderText = "TEST\r(s)";
            dataGridView1.Columns["finel"].HeaderText = "FINEL\r(mbar)";
            dataGridView1.Columns["finer"].HeaderText = "FINER\r(mbar)";
            dataGridView1.Columns["flow"].HeaderText = "FLOW";

            dataGridView1.Columns["合格品数量"].HeaderText = "合格品\r数量\r(只)";
            dataGridView1.Columns["不良品数量"].HeaderText = "不良品\r数量\r(只)";
            dataGridView1.Columns["判定"].HeaderText = "判定\r(合格划√\r不合格划×)";
        }
        ...
        private bool Save()
        {
            ...
            else
            {
                ...
                bs记录.EndEdit();
                da记录.Update((DataTable)bs记录.DataSource);
                readOuterData(InstruID, prodCode);
                outerBind();
        ...
        private void btn提交审核_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter da_temp = new SqlDataAdapter("select * from 待审核 where 表名='泄漏测试记录' and 对应ID=" + dt记录.Rows[0]["ID"], mySystem.Parameter.conn);
        ...
        private void fill_excel(Microsoft.Office.Interop.Excel._Worksheet mysheet, Microsoft.Office.Interop.Excel._Workbook mybook)
        {
            ...
            //mysheet.Cells[3, 12].Value = "生产日期：" + Convert.ToDateTime(dt记录.Rows[0]["生产日期"]).ToString("yyyy年MM月dd日");
            mysheet.Cells[3, 12].Value = "生产指令：" + dt记录.Rows[0]["生产指令编号"].ToString();

            mysheet.Cells[6, 3].Value = dt记录.Rows[0]["prefill"];
            mysheet.Cells[6, 4].Value = dt记录.Rows[0]["charde"];
            mysheet.Cells[6, 5].Value = dt记录.Rows[0]["settle"];
            mysheet.Cells[6, 6].Value = dt记录.Rows[0]["test"];
            mysheet.Cells[6, 7].Value = dt记录.Rows[0]["finel"];
            mysheet.Cells[6, 8].Value = dt记录.Rows[0]["finer"];
            mysheet.Cells[6, 9].Value = dt记录.Rows[0]["flow"];
            ...
            for (int i = 0; i < dt记录详情.Rows.Count; i++)
            {
                ...
                mysheet.Cells[7+ i, 6] = dt记录详情.Rows[i]["test"].ToString();
                mysheet.Cells[7 + i, 7] = dt记录详情.Rows[i]["finel"].ToString();
                mysheet.Cells[7 + i, 8] = dt记录详情.Rows[i]["finer"].ToString();
                mysheet.Cells[7 + i, 9] = dt记录详情.Rows[i]["flow"].ToString();

                mysheet.Cells[7 + i, 10] = dt记录详情.Rows[i]["合格品数量"];
                mysheet.Cells[7 + i, 11] = dt记录详情.Rows[i]["不良品数量"];
                mysheet.Cells[7 + i, 12] = dt记录详情.Rows[i]["判定"].ToString();
                mysheet.Cells[7 + i, 13] = dt记录详情.Rows[i]["操作员"].ToString();
                mysheet.Cells[7 + i, 14] = dt记录详情.Rows[i]["复核人"].ToString();
                mysheet.Cells[7 + i, 15] = dt记录详情.Rows[i]["备注"].ToString();

            }
            mysheet.Cells[22 + ind, 10].Value = dt记录.Rows[0]["合格品数量"];
            mysheet.Cells[22 + ind, 11].Value = dt记录.Rows[0]["不良品数量"];

            //mysheet.Cells[22 + ind, 10].Value = string.Format("合格品数量：    {0}只\n不良品数量：    {1}只", dt记录.Rows[0]["合格品数量"], dt记录.Rows[0]["不良品数量"]);
            //mysheet.Cells[22 + ind, 12].Value = string.Format("复核人： {0}\n日期：{1}", dt记录.Rows[0]["审核员"].ToString(), Convert.ToDateTime(dt记录.Rows[0]["审核日期"]).ToString("yyyy年MM月dd日"));
        }
        ...
        public int print(bool b)
        {
            ...
            Microsoft.Office.Interop.Excel._Workbook wb = oXL.Workbooks.Open(System.IO.Directory.GetCurrentDirectory() + @"\..\..\xls\PTV\11 SOP-MFG-415-R01A 泄漏测试记录.xlsx");
        ...
        private bool TextBox_check()
        {
            ...
            List<TextBox> TextBoxList = new List<TextBox>(new TextBox[] { tb不良品数量, tb合格品数量,
                tb参数1,tb参数2,tb参数3,tb参数4,tb参数5,tb参数6,tb参数7});
            List<String> StringList = new List<String>(new String[] { "不良品数量", "合格品数量",
                "PREFILL\r(%)","CHARDE\r(s)","SETTLE\r(s)","TEST\r(s)","FINEL\r(mbar)","FINER\r(mbar)","FLOW"});
            double numtemp = 0;
        ...
        private void PTVBag_testrecordofdisclose_FormClosing(object sender, FormClosingEventArgs e)
        {
