...
{
    public partial class PTVBag_innerpackaging : BaseForm
    {
        ...
        public PTVBag_innerpackaging(MainForm mainform)
            : base(mainform)
        {
            InitializeComponent();
        ...
        public PTVBag_innerpackaging(MainForm mainform, Int32 ID)
            : base(mainform)
        {
            InitializeComponent();
            ...
            dt代码批号 = new DataTable("代码批号");
                ...
                if (reader1.Read())
                {
                    ...
                    else
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            MessageBox.Show("该用户没有班次信息，请注意");
                            tb班次.Text = "未知";
                        }
                        else { tb班次.Text = dt.Rows[0]["班次"].ToString(); }
        ...
        void cb产品代码_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cb产品代码.Text != "")
            {
                tb生产批号.Text = dt代码批号.Rows[cb产品代码.FindString(cb产品代码.Text)]["产品批号"].ToString();
            }
        }
        ...
        private void readOuterData(Int32 InstruID, String productCode, DateTime datetime, string flight)
        {
            ...
            da记录 = new SqlDataAdapter(string.Format(sql, InstruID, productCode, datetime.ToString("yyyy/MM/dd"), flight), mySystem.Parameter.conn);
        ...
        private void PTVBag_innerpackaging_FormClosing(object sender, FormClosingEventArgs e)
        {
