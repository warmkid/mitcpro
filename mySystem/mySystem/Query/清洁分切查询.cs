            ...
            try
            {
                switch (tableName)
                {
                    case "清洁分切生产记录":
                        ...
                        { EachBind(this.dgv, "清洁分切生产记录", "操作人", "操作日期"); }
                        break;
                    case "清洁分切日报表":
                        if (comboBox1.SelectedIndex != -1)
                        { EachBind(this.dgv, "清洁分切日报表", "操作人", "操作日期", "生产指令ID"); }
                        else
                        { EachBind(this.dgv, "清洁分切日报表", "操作人", "操作日期"); }
                        break;
                    case "清洁分切开机前确认表":
                        if (comboBox1.SelectedIndex != -1)
                        { EachBind(this.dgv, "清洁分切开机确认", "确认人", "确认日期", "生产指令ID"); }
                        else
                        { EachBind(this.dgv, "清洁分切开机确认", "确认人", "确认日期"); }
                        break;
                    case "清洁分切运行记录":
                        if (comboBox1.SelectedIndex != -1)
                        { EachBind(this.dgv, "清洁分切运行记录", "确认人", "确认日期", "生产指令ID"); }
                        else
                        { EachBind(this.dgv, "清洁分切运行记录", "确认人", "确认日期"); }
                        break;
                    case "清场记录":
                        if (comboBox1.SelectedIndex != -1)
                        { EachBind(this.dgv, "清场记录", "清场人", "生产日期", "生产指令ID"); }
                        else
                        { EachBind(this.dgv, "清场记录", "清场人", "生产日期"); }   
                        break;
                    case "清洁分切批生产记录":
                        if (comboBox1.SelectedIndex != -1)
